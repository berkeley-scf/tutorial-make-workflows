...
If we wanted to run the full workflow, we just run `make` on the file:
```{r}
make
```
...
The pattern rule uses `%`, which acts as a wildcard and in this case tells make how to create targets such as *fig1.pdf*, *fig2.pdf*, ... from corresponding prerequisites *fig1.R*, *fig2.R*, etc. The `$<` and `$(@:.pdf=.Rout)` are special variables and substitutions described further later. Briefly `$<` refers to the target and `$(@:.pdf=.Rout)` substitutes .Rout for any cases of .pdf found in the target. 

You can also see rules for deleting interim files, the *clean* and *cleanall* targets.

# Using make for document preparation

The file *workshop_example/Makefile* is an example that illustrates how you might use make to prepare presentation materials for a workshop. It calls *workshop_example/modules/Makefile*, so take a look at that file.
...
Once again, I won't run make here because I don't have all the necessary dependencies in *workshop_example/modules*.
...
Another way to do substitution is with syntax like this, which replaces .pdf with .Rout in the `$@` variable. 
